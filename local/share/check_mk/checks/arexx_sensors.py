...
def check_arexx_sensors(item, params, info, sensor_type):
    ...
    for line in info:
        if line[0] == sensorid and line[4] in sensor_type and abs(localtime-int(line[2])) < 300:
...
def check_arexx_sensors_temp(item, params, info):
    ...
    if not value == None:
        return check_temperature(float(value),params, "arexx_temp_%s" % sensorid)
    
def check_arexx_sensors_humidity(item, params, info):
    value,description,sensorid=check_arexx_sensors(item, params, info,["Humidity","Relative Humidity"])
    if not value == None:
        return check_humidity(float(value),params)
...
check_info["arexx_sensors.humidity"] = {
    ...
    "inventory_function"      : lambda info: inventory_arexx_sensors(info,['Humidity','Relative Humidity']),
    'has_perfdata'            : True,
    "group"                   : "humidity",
